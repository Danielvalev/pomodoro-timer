...
class PomodoroTimer:
    def __init__(self):
        ...
        self.short_brake_timer_label = ttk.Label(self.tab2, text='05:00', font=('Ubuntu', 48))
    ...
    def start_timer(self):
        ...
        if timer_id == 1:
            ...
            while full_seconds > 0 and not self.stopped:
                minutes, seconds = divmod(full_seconds, 60)
                self.short_brake_timer_label.config(text=f'{minutes:02d}:{seconds:02d}')
                self.root.update()
                time.sleep(1)
                full_seconds -= 1
            if not self.stopped or self.skipped:
            ...
            while full_seconds > 0 and not self.stopped:
                minutes, seconds = divmod(full_seconds, 60)
                self.long_brake_timer_label.config(text=f'{minutes:02d}:{seconds:02d}')
                self.root.update()
                time.sleep(1)
                full_seconds -= 1
            if not self.stopped or self.skipped:
    ...
    def reset_clock(self):
        ...
        self.short_brake_timer_label.config(text='05:00')
        self.long_brake_timer_label.config(text='15:00')
